...
const InlineStyle = TextStyle.extend({
    ...
    return {
      ...
      fontSize: {
        ...
        parseHTML: (element) => {
          ...
          if (["h1", "h2", "h3", "h4", "h5", "h6"].includes(tagName)) {
            ...
            const headingStyle = headingStyles.find((h) => h.level === level);
            if (headingStyle) return headingStyle.fontSize;
          }
      ...
      fontWeight: {
        ...
        parseHTML: (element) => {
          ...
          // Check for bold tags
          const tagName = element.tagName.toLowerCase();
          if (["strong", "b"].includes(tagName)) return "bold";
      ...
      headingLevel: {
        ...
        parseHTML: (element) => {
          const tagName = element.tagName.toLowerCase();
          if (["h1", "h2", "h3", "h4", "h5", "h6"].includes(tagName)) {
            return tagName.charAt(1);
          }
          return element.getAttribute("data-heading-level");
        },
        renderHTML: (attributes) => {
  ...
  // Add parsing rules for better paste handling
  addPasteRules() {
    return [
      // Handle pasted headings
      {
        find: /<(h[1-6])[^>]*style="[^"]*font-size:\s*([^;"]+)[^"]*"[^>]*>(.*?)<\/\1>/gi,
        handler: ({ match, chain }) => {
          const level = Number.parseInt(match[1].charAt(1));
      ...
      // Handle pasted styled text
      {
        find: /<span[^>]*style="[^"]*font-size:\s*([^;"]+)[^"]*"[^>]*>(.*?)<\/span>/gi,
        handler: ({ match, chain }) => {
...
const headingStyles = [
  {
    ...
    fontWeight: "normal",
    label: "Heading 1",
    icon: <Icon icon="material-symbols:format-h1" className="h-4 w-4 mr-2" />,
  },
  {
    level: 2,
    fontSize: "1.5em",
    fontWeight: "normal",
    label: "Heading 2",
    icon: <Icon icon="material-symbols:format-h2" className="h-4 w-4 mr-2" />,
  },
  {
    level: 3,
    fontSize: "1.25em",
    fontWeight: "normal",
    label: "Heading 3",
    icon: <Icon icon="material-symbols:format-h3" className="h-4 w-4 mr-2" />,
  },
  {
    level: 4,
    fontSize: "1.1em",
    ...
    const editor = useEditor({
      ...
      extensions: [
        StarterKit.configure({
          ...
          // Configure paste handling to preserve formatting
          clipboardTextParser: {
            blockSeparator: "\n\n",
          },
        }),
          ...
          if (htmlData) {
            ...
            const processNode = (node) => {
              ...
              if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                if (style.includes("font-weight")) {
                  ...
                  if (fontWeightMatch) {
                    fontWeight = fontWeightMatch[1].trim();
                  }
                } else if (computedStyle) {
                ...
                if (["h1", "h2", "h3", "h4", "h5", "h6"].includes(tagName)) {
                  const level = Number.parseInt(tagName.charAt(1));
                  const headingStyle = headingStyles.find(
                    (h) => h.level === level
                  );
                  if (headingStyle) {
                    fontSize = headingStyle.fontSize;
                    attrs.headingLevel = level.toString();
                  }
                }
                ...
                if (
                  ...
                  tagName === "b" ||
                  fontWeight === "bold" ||
                  Number.parseInt(fontWeight) >= 600
                ) {
                ...
                // Add text style if we have formatting
                if (fontSize || fontWeight || color) {
                  const textStyleAttrs = {};
                  if (fontSize) textStyleAttrs.fontSize = fontSize;
                  if (fontWeight && fontWeight !== "normal")
                    textStyleAttrs.fontWeight = fontWeight;
                  if (color && color !== "rgb(0, 0, 0)" && color !== "#000000")
                    textStyleAttrs.color = color;
                  if (attrs.headingLevel)
                    textStyleAttrs.headingLevel = attrs.headingLevel;
                ...
                // For inline elements, return the content with marks
            ...
            try {
              ...
              for (let i = 0; i < tempDiv.childNodes.length; i++) {
                const result = processNode(tempDiv.childNodes[i]);
                if (result) {
              ...
              if (processedContent.length > 0) {
                ...
                const doc = {
                  type: "doc",
                  content: processedContent.filter(
                    (item) =>
                      item &&
                      (item.type === "paragraph" ||
                ...
                // Convert our custom format to ProseMirror format
                const schema = view.state.schema;
                try {
                  const proseMirrorDoc = schema.nodeFromJSON(doc);
                  tr.replaceWith(from, to, proseMirrorDoc.content);
                  view.dispatch(tr);
                  return true; // Prevent default paste behavior
                } catch (error) {
                  console.warn(
                    "Failed to parse pasted content, falling back to default behavior:",
                    error
                  );
                }
              }
            } catch (error) {
              console.warn("Error processing pasted HTML:", error);
            }
          }

          // If HTML processing failed or no HTML data, fall back to default behavior
          return false;
        },
        attributes: {
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-0.5 p-1 w-[98.5%] bg-white editorShadow rounded-md absolute top-2 left-2 z-[1000]">
            ...
            <div className="flex items-center">
              ...
              >
                <Icon icon="material-symbols:undo" className="size-5" />
              </button>
              <button
                type="button"
              ...
              >
                <Icon icon="material-symbols:redo" className="size-5" />
              </button>
            </div>

            {/* Paragraph button and Heading dropdown */}
            <div className="flex items-center border-r border-l border-[#E7E7E7] px-1 w-[3.7rem]">
              <div className="relative w-full" ref={headingMenuRef}>
                <button
                  type="button"
                  onClick={() => setIsHeadingMenuOpen(!isHeadingMenuOpen)}
                  className={cn(
                    "p-1.5 w-full rounded hover:bg-gray-200 flex items-center font-medium justify-between text-lg",
            ...
            {/* Font size dropdown */}
            <div
              className="relative flex items-center border-r border-[#E7E7E7] pr-1 mr-1"
              ref={fontSizeMenuRef}
            >
              <button
                type="button"
                ...
                className={cn(
                  "p-1.5 rounded hover:bg-gray-200 flex items-center min-w-20 justify-between font-medium",
              ...
              {isFontSizeMenuOpen && (
                <div
                  ...
                  style={{ top: "100%", left: "0", width: "160px" }}
                >
                  {fontSizes.map((size) => (
                    <button
                      key={size.value}
            ...
            {/* Image upload */}
            <div className="flex items-center border-r border-[#E7E7E7] pr-1 mr-1">
              <button
                type="button"
                ...
                disabled={disabled}
              >
                <Icon icon="material-symbols:image" className="h-4 w-4 mr-1" />
            ...
            <div className="flex items-center border-r border-[#E7E7E7] pr-1 mr-1">
              <button
                ...
                disabled={disabled}
              >
                <Icon icon="material-symbols:format-bold" className="h-4 w-4" />
              </button>
              <button
                type="button"
              ...
              <button
                ...
                disabled={disabled}
              >
                <Icon icon="material-symbols:code" className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="flex items-center border-r border-[#E7E7E7] pr-1 mr-1">
              <div className="relative">
                <button
                  ...
                  disabled={disabled}
                >
                  <Icon icon="material-symbols:link" className="h-4 w-4 mr-1" />
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex">
                        <input
                          ...
                          className="flex-1 px-2 py-1.5 text-sm border rounded-l focus:outline-none focus:border-blue-500"
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              handleLinkInsert();
                            }
