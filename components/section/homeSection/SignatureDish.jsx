...
export default function SignatureDish() {
  return (
    <div className="py-16">
      ...
      <div className="grid md:grid-cols-2 gap-5 container mx-auto 2xl:px-40 px-4">
        {signatureDishMenuItems.map((item, index) => (
          <div
            key={index}
            className=" bg-[#152225] rounded-xl p-2 flex items-center gap-5 relative"
          >
            <div className=" w-60 aspect-square h-52 bg-red-400/10 rounded-md center ">
              <Icon
                icon={item.icon}
